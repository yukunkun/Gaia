 ...
 */
public class ScripeGropDialog extends DialogFragment{
    ...
    public static ScripeGropDialog newInstance(long cid, int language) {
        ScripeGropDialog fragment = new ScripeGropDialog();
    ...
    private void getInfo() {
        MJsonHttpResponseHandler handler=new MJsonHttpResponseHandler(ScripeGropDialog.class){
            @Override
    ...
    private void addVideoToGroup(long id) {
        MJsonHttpResponseHandler handler = new MJsonHttpResponseHandler(ScripeGropDialog.class) {
            ...
            public void onBadResponse(JSONObject response) {
                ...
                if (response.optInt("i") == 37614) {
                    GaiaApp.showToast("剧本已存在");
                }else {
        ...
        //cid 就是sid.没有改而已
        GroupApiHelper.addVideoToGroup(id, cid, 2, getActivity(), handler);
    }
}
