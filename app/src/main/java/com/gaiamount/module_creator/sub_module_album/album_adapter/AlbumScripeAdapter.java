 ...
 */
public class AlbumScripeAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    Context context;
    ArrayList <ScripeInfo> scripeInfos;
    private long uid;
    private Info info;

    public AlbumScripeAdapter(Context context, ArrayList<ScripeInfo> scripeInfos, Info info) {
    ...
    private void showPopu(ImageView view, long sid, int position) {
        ...
        //在控件上方显示
        popupWindow.showAsDropDown(view,0-popupWidth+ScreenUtils.dp2Px(context, 10),0-popupHeight- ScreenUtils.dp2Px(context, 30));
    }
    ...
    private void removeVideo(long cid, final int position, Context context) {
        ...
        AlbumApiHelper.removeVideoFromAlbum(id1, info.gid,2,context, handler); //2 才是剧本
