 ...
 */
public class MaterialAlbumAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public MaterialAlbumAdapter(Context context, ArrayList<MaterialInfo> mMaterialInfoList, Info info) {
...
//                ImageUtils.getInstance(mContext).getNetworkBitmap(holder.workCover, cover);
                cover = Configs.COVER_PREFIX+cover /*+ cover.replace(".", "_18.")*/;
...
//                        (location[0] + view.getWidth() / 2) - popupWidth*2, location[1] - popupHeight*4);
        popupWindow.showAsDropDown(view,0-popupWidth-ScreenUtils.dp2Px(mContext, 1),0-popupHeight-ScreenUtils.dp2Px(mContext, 20));
    }
    ...
    private void removeVideo(long cid, final int position, Context context) {
        ...
        AlbumApiHelper.removeVideoFromAlbum(id1, info.gid,1,context, handler);
