 ...
 */
public class AlbumScriptDialog extends DialogFragment{
    ...
    private ScripeInfo mList;

    public static AlbumScriptDialog newInstance(long gid, ScripeInfo mList, int position) {
        AlbumScriptDialog fragment = new AlbumScriptDialog();
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        mList= (ScripeInfo) arguments.getSerializable("mList");
    }
    ...
    private void getInfo() {
        MJsonHttpResponseHandler handler = new MJsonHttpResponseHandler(AlbumScriptDialog.class) {
            @Override
    ...
    private void addGroup(long aid) {
        ...
        Long[] wids = new Long[]{Long.valueOf(mList.getSid())};
        MJsonHttpResponseHandler handler = new MJsonHttpResponseHandler(AlbumScriptDialog.class){
            @Override
            public void onGoodResponse(JSONObject response) {
                super.onGoodResponse(response);
        ...
        AlbumApiHelper.addVideoToAlbum(wids,2,aid,getActivity(),handler);//type 2才是剧本
    }
}
