...
public class MaterialPlayActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        materialType = getIntent().getIntExtra("materialType",0);

        mDecorView = getWindow().getDecorView();

        //做一些初始化的操作
        toolbar = (Toolbar) findViewById(R.id.toolbar);
        mMediaController = new GMaterialController(this);
        mMediaController.setVideoControl(videoControl);
        mMediaController.setToolbar(toolbar);
        frameLayout = (FrameLayout) findViewById(R.id.frame_layouty);
        // init player
        IjkMediaPlayer.loadLibrariesOnce(null);
        IjkMediaPlayer.native_profileBegin("libijkplayer.so");

        mVideoView = (GVideoMaterialView) findViewById(R.id.gaia_video_view);
        ...
        MaterialVideoUtil videoUtil = new MaterialVideoUtil();
        //获取视频详细信息
        videoUtil.getVideoInfo(this, mid);
    ...
    private void initToolBar(Toolbar toolbar) {
        ...
        MenuItem item_set = toolbar.getMenu().findItem(R.id.action_work_setting);
        item_set.setVisible(false);
//        MenuItem item_load = toolbar.getMenu().findItem(R.id.action_download);
//        item_load.setVisible(false);
        if(isCollected){
...
//                        ActivityUtil.startWorkSettingActivity(MaterialPlayActivity.this, mWorkId, mContentType);
//                    } else {
//                        GaiaApp.showToast(getString(R.string.no_power));
//                    }

                } else if (itemId == R.id.action_share) {
                    ShareUtil.newInstance(getApplicationContext()).share(mDetailInfo.getMaterial().getName(),"分享自Gaiamount的素材",
                            Configs.COVER_PREFIX+mDetailInfo.getMaterial().getScreenshot()+".png" ,"https://gaiamount.com/footage/detail/"+mDetailInfo.getMaterial().getMid());
                    ...
                    MaterialGropDialog commentDialog=MaterialGropDialog.newInstance(mDetailInfo.getMaterial().getMid(),1);//mid传递过去之后,就是cid的值
                    commentDialog.show(getSupportFragmentManager(), "MaterialGropDialog");
                }
                return true;
            }
        });
        ...
        mTitle.requestFocus();
        String strings[]=getApplicationContext().getResources().getStringArray(R.array.player_tabs);
    ...
    private void collectionWorksToggle(final int collection, final Drawable drawable) {
        ...
        JsonHttpResponseHandler jsonHttpResponseHandler = new MJsonHttpResponseHandler(MaterialPlayActivity.class, progressDialog) {
            @Override
        ...
        MaterialApiHelper.getMaterialAddColl(GaiaApp.getAppInstance().getUserInfo().id,mid,collection,getApplicationContext(),jsonHttpResponseHandler);
    }
...
//    private void prepareVideo(Uri defaultUri) {
////        Uri uri = Uri.parse(defaultUri);
////        Log.i("----def",uri.getPath());
//        mVideoView.setVideoURI(defaultUri);
////        Log.i("----def",defaultUri.toString());
//        //下面两句顺序不能对调
//        mMediaController.setDefaultUri(defaultUri.getPath());
//        mMediaController.setVideoInfo(mDetailInfo);
//
//    }

    @Subscribe(threadMode = ThreadMode.MAIN)
    public void onEventAfterGetData(MaterialVideoUtil.OnVideoInfoGettedEvent event) {
        mDetailInfo = event.getDetailInfo();
        int isCollect = mDetailInfo.getMaterial().getIsCollect();
        //收藏
        if(isCollect==0){
            isCollected=false;
        }else {
        ...
        //设置标题
        if (mTitle != null) {
            mTitle.setText(mDetailInfo.getMaterial().getName());
        }
        //隐藏下部分中的progressBar
...
//        }
        ...
        RecomendFragment recFrag = new RecomendFragment();

        detailFragment.setArguments(bundle);
        recFrag.setArguments(bundle);
        List<Fragment> fragmentList = new ArrayList<>();
        fragmentList.add(detailFragment.newInstance(mDetailInfo, mid,materialType));
        fragmentList.add(new CommentFragment().newInstance(mDetailInfo,mid));
        fragmentList.add(recFrag.newInstance(mid));
    ...
    private IMediaPlayer.OnCompletionListener onComplietionListener = new IMediaPlayer.OnCompletionListener() {
        ...
        public void onCompletion(IMediaPlayer mp) {
            ...
            //准备图片
//           ImageUtils.convertToBlur(MaterialPlayActivity.this, mDetailInfo.getVideoInfo().getScreenshot(), mDetailInfo.getVideoInfo().getScreenshot());
...
//        mManager = new HistoryDBManager(MaterialPlayActivity.this);
//        mManager.add(mDetailInfo.getVideoInfo().getScreenshot(), mDetailInfo.get().getCover(), mDetailInfo.getWorks().getName(), System.currentTimeMillis(), mWorkId, mContentType);
    }
